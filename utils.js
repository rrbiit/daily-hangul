    ...
    function generateExample(w) {
      ...
      // ─── 已确认失效的词类（生成器语义错误，必须整课手写）───
      // 第3~5课逐课 dump 后确认以下类别生成的例句是病句，不能作为学习内容：
      // 1) 时间/星期/周次类名词（지금/오늘/어제/월요일/지난주/여행/기분 等）
      //    → 全部掉进「物件」模板：새 지금을 샀어요(买了新的现在)、어제 오늘을 잃어버렸어요
      // 2) 三餐词（아침/점심/저녁）→ 被套「食物」模板，完全没体现时间用法
      // 3) 词组（이를 닦다/옷을 입다 等）→ 整个词组被当名词加 를：이를 닦다를 자주 합니다
      // 4) 依存名词默认模板（非 씨/시/분/층/호선 等特判项）→ 语义无关、生硬
      // 5) 部分动词模板有语法病句：처음 일어났습니다 때…(-습니다 不能接 때)、시간이 일어나면…
      // 结论：主题课一律整课手写（w.ex），生成器只作「语法正确」的兜底，不追求语义正确。
      // 若要对上述词类新增模板，请先 dump 校验再上线，勿直接补硬编码。
      // ────────────────────────────────────────────────────────
